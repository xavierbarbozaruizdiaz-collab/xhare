-- Atomicidad de agrupación:
-- Cuando un pedido pasa a status='grouped', también se escribe trip_requests.ride_id = group_id
...
BEGIN
  ...
  SET status = 'grouped', ride_id = p_group_id, updated_at = now()
  WHERE id = p_trip_request_id;
...
-- ---------------------------------------------------------------------------
-- Flush corridor bucket: grupo nuevo + membresías (compatibilidad v1/v2)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  UPDATE public.trip_requests
  SET status = 'grouped', ride_id = v_gid, updated_at = now()
  WHERE id = ANY (p_ids);
