-- Nombre opcional del viaje (lo define el conductor); visible en listados y búsqueda del pasajero.

ALTER TABLE public.rides
  ADD COLUMN IF NOT EXISTS route_name text;

COMMENT ON COLUMN public.rides.route_name IS
  'Nombre corto del viaje (ej. Ruta Centro–Luque). Opcional; búsqueda en app pasajero.';

-- Incluir route_name en el JSON del detalle (app / reservar).
...
COMMENT ON FUNCTION public.get_ride_detail_for_user(uuid) IS
  'Detalle de un ride para conductor, admin, pasajero con reserva o cualquier usuario si status=published (para reservar). Incluye route_name.';
